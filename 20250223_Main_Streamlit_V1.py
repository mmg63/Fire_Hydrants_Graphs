...
from utils import get_hydrant_metric_space
...
folium.GeoJson(hazard_gdf.to_crs(epsg=4326), name="Fire Hazard Zones", style_function=lambda x: {"fillColor": "blue", "color": "black", "weight": 1}).add_to(m)
for _, row in hydrant_gdf.to_crs(epsg=4326).iterrows():
    folium.CircleMarker(location=[row.geometry.y, row.geometry.x], radius=1, color='red', fill=True, fill_color='red').add_to(m)
st_folium(m)
...
HeatMap(hydrant_latlon, radius=12, blur=10, max_zoom=1).add_to(m_heat)
st_folium(m_heat)
...
st.subheader("📏 Hydrant Distance Matrix")
utm_coordinates = get_hydrant_metric_space(hydrant_gdf)
distance_matrix = pd.DataFrame(
squareform(pdist(utm_coordinates)),
columns=[f"H{i+1}" for i in range(len(utm_coordinates))],
index=[f"H{i+1}" for i in range(len(utm_coordinates))]
